  ...
  return (
    <View className="flex-1 bg-slate-900 px-4 pt-4">
      ...
      <View className="bg-slate-800 rounded-2xl p-4 mb-4 border border-slate-700">
        <Text className="text-white font-semibold mb-3">Add Team</Text>
        <Input
          label="Team Name"
        ...
        renderItem={({ item }) => (
          <View className="bg-slate-800 rounded-xl p-4 mb-3 border border-slate-700">
            <View className="flex-row items-center justify-between mb-3">
              ...
              <Text className="text-slate-500 text-xs">{item._count.members} members</Text>
            </View>
            {item.members.map((m) => (
              <View key={m.id} className="flex-row items-center justify-between py-1.5 border-t border-slate-700">
                <Text className="text-slate-300 text-sm">{m.user.name}</Text>
            ...
            {item.members.length === 0 && (
              <Text className="text-slate-600 text-sm text-center py-2">No members yet</Text>
            )}
          </View>
        )}
        ListEmptyComponent={
          !isLoading ? (
            <Text className="text-slate-600 text-center py-8">No teams yet. Create one above.</Text>
          ) : null
        }
      />
    </View>
